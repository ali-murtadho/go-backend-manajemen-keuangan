...
-- dengan banyak tb transaksi
-- Master Waktu
CREATE TABLE m_waktu (
    id SERIAL PRIMARY KEY,
    bulan_tahun VARCHAR(7) NOT NULL -- format MM-YYYY
);
...
CREATE TABLE t_simpanan_waktu_bulanan (
    id SERIAL PRIMARY KEY,
    id_simpanan INT REFERENCES m_simpanan(id) ON DELETE CASCADE,
    id_waktu INT REFERENCES m_waktu(id) ON DELETE CASCADE,
...
-- Pemasukan bulanan
CREATE TABLE t_pemasukan_bulanan (
    id SERIAL PRIMARY KEY,
    id_simpanan INT REFERENCES m_simpanan(id) ON DELETE CASCADE,
    id_waktu INT REFERENCES m_waktu(id) ON DELETE CASCADE,
    judul VARCHAR(150) NOT NULL,
    biaya NUMERIC(15,2) NOT NULL,
    pembayaran VARCHAR(50), -- metode pembayaran
    tanggal_bayar DATE,
    isPaid BOOLEAN DEFAULT FALSE
);

-- Pengeluaran bulanan
CREATE TABLE t_keluaran_bulanan (
    id SERIAL PRIMARY KEY,
    id_simpanan INT REFERENCES m_simpanan(id) ON DELETE CASCADE,
    id_waktu INT REFERENCES m_waktu(id) ON DELETE CASCADE,
    judul VARCHAR(150) NOT NULL,
...
-- Pengeluaran harian
CREATE TABLE t_keluaran_harian (
    id SERIAL PRIMARY KEY,
    id_simpanan INT REFERENCES m_simpanan(id) ON DELETE CASCADE,
    id_waktu INT REFERENCES m_waktu(id) ON DELETE CASCADE,
...
-- Transaksi umum (pemasukan, pengeluaran, saving)
CREATE TABLE t_transaksi (
    ...
    judul VARCHAR(150) NOT NULL,
    biaya NUMERIC(15,2) NOT NULL,
    jenis VARCHAR(20) NOT NULL, -- 'pemasukan' atau 'pengeluaran'
    tipe VARCHAR(20),           -- 'bulanan', 'harian', 'saving'
    pembayaran VARCHAR(50),
    catatan TEXT,
    isPaid BOOLEAN DEFAULT FALSE
);
